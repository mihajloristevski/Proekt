...
namespace Proekt
{
    public partial class Izbrishi_Prodavnica : Form
    {
        public Izbrishi_Prodavnica()
        {
            InitializeComponent();
        }
        ...
        private void Izbrishi_Prodavnica_Load(object sender, EventArgs e)
        {
            this.Width = 640;
            this.Height = 600;
            this.BackColor = Color.FromArgb(244, 66, 66);
            this.Text = "Избриши Продавница";
            this.ControlBox = false;


            Label lbprikaz = new Label();
            LabelZ lbprikazi = new LabelZ(50, 50, 350, 12, "Приказ на податоци во Продавница:", lbprikaz);
            ...
            dgvArtikal.Height = 170;
            dgvArtikal.Location = new Point(50, 90);

            dgvArtikal.DataSource = bindingSource;
            conn.Open();
            dataAdapter = new SqlDataAdapter("select id_artikal AS РеденБрој, Ime AS Име, Tezina AS Тежина, Cena AS Цена,Kolicina AS Количина from Artikal where (id_podklasa < 3)", conn);
