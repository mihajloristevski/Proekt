...
namespace Proekt
{
    public partial class Izbrishi_Kafic : Form
    {
        public Izbrishi_Kafic()
        {
            InitializeComponent();
        }
        ...
        private void Izbrishi_Kafic_Load(object sender, EventArgs e)
        {
            this.Width = 640;
            this.Height = 600;
            this.BackColor = Color.FromArgb(244, 66, 66);
            this.Text = "Избриши кафич";
            this.ControlBox = false;

            Label lbprikaz = new Label();
            LabelZ lbprikazi = new LabelZ(50, 50, 350, 12, "Приказ на податоци во кафич:", lbprikaz);
            ...
            conn.Open();
            dataAdapter = new SqlDataAdapter("select id_artikal AS РеденБрој, Ime AS Име, Tezina AS Тежина, Cena AS Цена,Kolicina AS Количина from Artikal where (id_podklasa between 7 and 9)", conn);
